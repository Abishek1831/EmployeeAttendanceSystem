...
switch ($action) {
    case 'apply_leave':
        if ($_SESSION['user_type'] == 'employee') {
            ...
            // SECURITY: $leaveType is used to build a column name (e.g. "sick_leave")
            // below. Escaping a value is NOT enough to make it safe as an identifier,
            // so it must be checked against a fixed whitelist before use.
            $allowedLeaveTypes = ['sick', 'casual', 'earned'];
    ...
    case 'approve_leave':
        if ($_SESSION['user_type'] == 'admin') {
            ...
            if ($conn->query($updateSql)) {
                ...
                if ($status == 'approved') {
                    // NOTE: leave_type is read back from the database, not from user
                    // input, and is only ever written by apply_leave above (which now
                    // whitelists it), so this is safe as long as no other code path
                    // inserts leave_applications rows with an unvalidated leave_type.
